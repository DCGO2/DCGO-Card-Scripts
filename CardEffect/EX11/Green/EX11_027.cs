        ...
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                IEnumerator ActivateCoroutine(Hashtable hashtable)
                {
                    ...
                    if (CardEffectCommons.HasMatchConditionOwnersPermanent(CanMaybeLinkPermanent))
                    {
                        ...
                        int index = 1;
                        bool canLinkHand = CardEffectCommons.HasMatchConditionOwnersHand(CanSelectCardCondition);
                        if (canLinkHand)
                        {
                        ...
                        if (doLink)
                        {
                            ...
                            if (selectedCard != null || !fromHand)
                            {
                                ...
                                int maxCount = Math.Min(1, CardEffectCommons.MatchConditionPermanentCount(permanent => CanSelectLinkTarget(permanent, selectedCard ?? card)));

                                SelectPermanentEffect selectPermanentEffect = GManager.instance.GetComponent<SelectPermanentEffect>();

                                selectPermanentEffect.SetUp(
                                    selectPlayer: card.Owner,
                                    canTargetCondition: permanent => CanSelectLinkTarget(permanent, selectedCard ?? card),
                                    canTargetCondition_ByPreSelecetedList: null,
                                    canEndSelectCondition: null,
                                    maxCount: maxCount,
                                    canNoSelect: false,
