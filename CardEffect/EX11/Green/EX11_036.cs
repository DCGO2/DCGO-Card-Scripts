    ...
    public class EX11_036 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OnEndTurn)
            {
                ...
                {
                    return CardEffectCommons.IsExistOnBattleAreaDigimon(card)
                        && CardEffectCommons.HasMatchConditionOwnersPermanent(CanDigivolvePermanentCondition);
                }

                bool CanDigivolvePermanentCondition(Permanent permanent)
                {
                    return permanent != card.PermanentOfThisCard()
                        && CardEffectCommons.HasMatchConditionOwnersHand(cardSource => CanDigivolveCardCondition(cardSource, permanent));
                }
            ...
            if (timing == EffectTiming.WhenLinked)
            {
                ...
                bool PermamentCondition(Permanent permanent) => permanent == card.PermanentOfThisCard();
                ...
                bool IsOpponentsDigimon(Permanent permanent) => CardEffectCommons.IsPermanentExistsOnOpponentBattleAreaDigimon(permanent);
