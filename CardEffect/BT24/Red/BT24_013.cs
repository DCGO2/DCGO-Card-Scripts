...
namespace DCGO.CardEffects.BT24
{
    public class BT24_013 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            {
                return CardEffectCommons.IsPermanentExistsOnOpponentBattleAreaDigimon &&
            ...
            if (timing == EffectTiming.OnAllyAttack)
            {
                ActivateClass activateClass = new ActivateClass();
                activateClass.SetUpICardEffect(SharedEffectName(), CanUseCondition, card);
                activateClass.SetUpActivateClass(SharedCanActivateCondition, SharedActivateCoroutine, 1, false, SharedEffectDescription("When Attacking"));
                activateClass.SetHashString = SharedHash();
                cardEffects.Add(activateClass);
            ...
            if (timing == EffectTiming.OnDiscardHand)
            {
                ...
                string EffectDescription()
                {
                    return "[Your Turn] [Once Per Turn] When your hand is trashed from, this [Demon] or [Titan]";
                }
