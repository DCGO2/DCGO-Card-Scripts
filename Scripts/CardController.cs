...
{
    public IBattle(Permanent AttackingPermanent, Permanent DefendingPermanent, CardSource DefendingCard, ICardEffect CardEffect = null)
    {
        this.AttackingPermanent = AttackingPermanent;
    ...
    public IEnumerator Battle()
    {
        ...
        if (AttackingPermanent != null)
        {
            ...
            if (IsExistingDefender())
            {
                ...
                if (CardEffect == null)//If started by effect, do not perform processing during this battle
                {
                    // auto process check
                    yield return ContinuousController.instance.StartCoroutine(GManager.instance.autoProcessing.AutoProcessCheck());
        ...
        if (CardEffect == null)//If started by effect, do not perform processing & don't reset effects relevant to the actual attack process
        {
            // auto process check
            yield return ContinuousController.instance.StartCoroutine(GManager.instance.autoProcessing.AutoProcessCheck());
